<template>
  ...
  <div class="box">
    <div class="form">
      ...
      <!-- 提交 -->
      <div class="submit" @click="submitInfo">保存</div>
    </div>
  </div>
</template>
<script>
    ...
    submitInfo() {
      if (
        ...
        let info = {
          ...
          applicantBasicSex: this.sex.map((sitem) => sitem).indexOf(this.value),
          applicantBasicNationality: this.value2,
          applicantBasicIsdualnationality: this.DualNationality.map(
            (sitem) => sitem
          ).indexOf(this.value3),
